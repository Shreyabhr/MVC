...
namespace Bank.BL
{
    public class BankADORepository : IBankRepository
    {

        private SqlConnection _sqlConnection;

        public BankADORepository()
        {
        ...
        public void DoTrasanction(string name, string password, double amount, char type)
        {
            ...
            try
            {
                if (type == 'D')
                {
                    updateCommand = new SqlCommand("UPDATE BANK_MASTER SET BALANCE = BALANCE + @amount WHERE NAME lIKE @name AND PASSWORDHASH = HASHBYTES('MD5',@password)", _sqlConnection,transaction);
                }
                else if (type == 'W')
                {
                    updateCommand = new SqlCommand("UPDATE BANK_MASTER SET BALANCE = BALANCE - @amount WHERE NAME lIKE @name AND PASSWORDHASH = HASHBYTES('MD5',@password)", _sqlConnection,transaction);
        ...
        public string Login(string name, string password)
        {
            ...
            var loginCommand = new SqlCommand("SELECT NAME FROM BANK_MASTER WHERE NAME = @name and PASSWORDHASH = HASHBYTES('MD5',@password)", _sqlConnection);
